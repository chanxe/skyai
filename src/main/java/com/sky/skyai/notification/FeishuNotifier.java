...
/**
 * Feishu (飞书) notifier for Spring Boot Admin
...
public class FeishuNotifier extends AbstractEventNotifier {
    ...
    @Value("${spring.boot.admin.notify.feishu.webhook-url:}")
    private String webhookUrl;

    @Value("${spring.boot.admin.notify.feishu.enabled:false}")
    ...
    protected Mono<Void> doNotify(InstanceEvent event, Instance instance) {
        return Mono.fromRunnable(() -> {
            if (!enabled || webhookUrl == null || webhookUrl.isEmpty()) {
                log.debug("Feishu notification is disabled or webhook URL is not configured");
                return;
            }
